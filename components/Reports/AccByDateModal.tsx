...
import { setAccByDateModalVisible, setFeesModalVisible } from "@/redux/reducer/reportSlice";
...
const AccByDateModal = () => {
  const [date, setDate] = useState(new Date());
  const [date2, setDate2] = useState(new Date());
  const [open, setOpen] = useState(false);
  const [open2, setOpen2] = useState(false);

  const { AccByDateModalVisible } = useSelector(
  ...
  const handleSearch = () => {
    ...
    router.navigate("/AccReportByDate");
    ...
    <>
      ...
      >
        ...
        >
          <TouchableOpacity
            activeOpacity={1}
            onPress={() => dispatch(setAccByDateModalVisible())}
            style={{
              position: "absolute",
              height: "100%",
              width: "100%",
